...
#define R_EN  26
#define L_PWM 25
#define R_PWM_L 33   // R_PWM des linken  BTS7960
...
if (abs(leftY) < DEADZONE) {
    ...
    // Stick oben → vorwärts
    int speed = map(abs(leftY), DEADZONE, 511, 0, 255);
    ledcWrite(CH_L_FWD, speed);
    ledcWrite(CH_L_BWD, 0);
} else {
    // Stick unten → rückwärts
    int speed = map(leftY, DEADZONE, 511, 0, 255);
    ledcWrite(CH_L_FWD, 0);
...
if (abs(rightY) < DEADZONE) {
    ...
    // Stick oben → vorwärts
    int speed = map(abs(rightY), DEADZONE, 511, 0, 255);
    ledcWrite(CH_R_FWD, speed);
    ledcWrite(CH_R_BWD, 0);
} else {
    // Stick unten → rückwärts
    int speed = map(rightY, DEADZONE, 511, 0, 255);
    ledcWrite(CH_R_FWD, 0);
